...
int Sigmet_ToDorade(struct Sigmet_Vol *vol_p, int s, struct Dorade_Sweep *swp_p)
{
    ...
    radd_p->radar_longitude = Sigmet_Bin4Rad(vol_p->ih.ic.longitude) * DEG_PER_RAD;
	...
	strncpy(parm_p->parameter_name, Sigmet_DataType_Descr(vol_p->types[p]), 40);
	...
	ryib_p->azimuth
	    = DEG_PER_RAD * 0.5 * (vol_p->ray_az0[s][r] + vol_p->ray_az1[s][r]);
	ryib_p->elevation
